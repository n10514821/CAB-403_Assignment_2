...
void List_creation( Combined_Temperature_list_t* vec ) {
   ...
   vec->Smoothe_Temperature_storage=malloc(0);
...
void Temperature_System (Combined_Temperature_list_t* vec, int16_t Temperature ){
//List variables
...
if (New_smoothe_temperature_length ==1){
    printf("Start of ST %d \n",Current_smoothe_temperature_length);
...
//print_all_Recored_temps(&Level_1_Temperature);

if(starter >=6){
    //print_all_Smooth_temps(&Level_1_Temperature);
}
